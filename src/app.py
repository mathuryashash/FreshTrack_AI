...
with st.sidebar:
    ...
    selected_city = st.selectbox(
        ...
        city_options,
        help="Auto-fetch real-time temperature and humidity",
    )
    ...
    if city:
        try:
            ...
            if w_res.get("main"):
                ...
                st.success(
                    f"Fetched: {city} ({st.session_state['temp']}°C, {st.session_state['humidity']}%)"
                )
            else:
    ...
    temp = st.slider(
        ...
        50.0,
        float(default_temp),
        help="Ambient temperature of the storage area",
    )
    humidity = st.slider(
        "Humidity (%)", 0.0, 100.0, float(default_hum), help="Relative humidity level"
    )

    st.markdown("---")
    st.info(
        "💡 **Tip:** Clear lighting and high-resolution images improve prediction accuracy."
    )

# Header
st.title("🍎 Fruit Freshness & Quality Analyzer")
st.markdown(
    "Predict freshness level, quality grade, and estimated shelf-life using multi-task deep learning."
)
...
with col1:
    ...
    uploaded_file = st.file_uploader(
        "Upload an image of the fruit...", type=["jpg", "jpeg", "png"]
    )
    ...
    if "prediction_result" in st.session_state:
        ...
        <div class="metric-container">
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{res["quality"]}</div>
            </div>
        </div>
        <div class="metric-container">
            <div class="metric-card">
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{res["freshness_confidence"] * 100:.1f}%</div>
            </div>
        </div>
        """,
            unsafe_allow_html=True,
        ...
        with st.expander("Show detailed confidence scores", expanded=True):
            st.write("### Freshness Confidence")
            conf = res["freshness_confidence"]
        ...
        if res["freshness"] == "Fresh":
            st.success(
                "🌟 This fruit is in peak condition! Can be stored at room temperature or refrigerated to extend life further."
            )
        elif res["freshness"] == "Semi-ripe":
            st.info("📅 nearing optimal ripeness. Best consumed within 2-3 days.")
        elif res["freshness"] == "Overripe":
            st.warning(
                "⚠️ Consume immediately or use for processed products (smoothies/jams). Do not store further."
            )
        else:
    ...
    if "prediction_result" in st.session_state:
        ...
        with st.expander("Submit Feedback / Correct Prediction"):
            with st.form("feedback_form"):
                st.write(
                    "If you think the prediction was incorrect, please provide the actual values."
                )
                ...
                f_shelf_life = st.number_input(
                    "Correct Shelf-Life (Days)", 0.0, 30.0, value=0.0
                )
                ...
                if submit_feedback:
                    ...
                    feedback_payload = {
                        "prediction_id": st.session_state.get(
                            "last_prediction_id", "N/A"
                        ),
                    ...
                    try:
                        f_res = requests.post(feedback_url, json=feedback_payload)
                        if f_res.status_code == 200:
                            st.success(
                                "Thank you for your feedback! It has been logged for model retraining."
                            )
                        else:
...
with footer_col2:
    st.markdown(
        "<p style='text-align: right;'><a href='#'>Documentation</a> | <a href='#'>GitHub Repo</a></p>",
        unsafe_allow_html=True,
    )
